...
namespace ConsoleControlLibrary.Controls
{
    public class Checkbox : ControlBase
    {
        ...
        public Checkbox(ConsoleForm parentForm, IDrawEngine drawEngine, bool isChecked, int x, int y, int width, int height, string text) : base(parentForm, drawEngine, x, y, width, height)
        {
            if (width < 3)
        ...
        public Checkbox(ConsoleForm parentForm, IDrawEngine drawEngine, bool isChecked, int x, int y, int width, string text) : this(parentForm, drawEngine, isChecked, x, y, width, 1, text) { }
        public Checkbox(ConsoleForm parentForm, IDrawEngine drawEngine, bool isChecked, int x, int y, string text) : this(parentForm, drawEngine, isChecked, x, y, text.Length + 3, 1, text) { }
        public bool Checked
        {
            get => _checked;
            set
        ...
        private string VisibleText => _visibleText ?? (_visibleText = (Checked ? "[X]" : "[ ]") + (_text.Length <= Width - 3 ? _text : _text.Substring(0, Width - 3)));
